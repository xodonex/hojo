 ...
 */
public class ArrayIndexRangeExpr extends IndexRangeExpr {

    private static final long serialVersionUID = 1L;

    public ArrayIndexRangeExpr(Expression a, Expression lo, Expression hi) {
        super(a, lo, hi);
    }

    @Override
    public Object xeq(Environment env) {
        Object a = ConvertUtils.toArray(e1.xeq(env));
        int lo = ConvertUtils.toInt(e2.xeq(env));
        int hi = ConvertUtils.toInt(e3.xeq(env));
        return (lo > hi) ? new PrimitiveArrayList(a, lo)
                : new PrimitiveArrayList(a, lo, hi);
    }
